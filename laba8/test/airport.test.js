...
const Airport = require('../Planes/Airport');
...
describe('Airport Tests', () => {
    ...
    it('should have military planes with transport type', () => {
        ...
        const hasTransportMilitaryPlanes = transportMilitaryPlanes.some(plane => plane.getMilitaryType() === MilitaryType.transport);
    ...
    it('should check passenger plane with max capacity', () => {
        ...
        assert.notStrictEqual(expectedPlaneWithMaxPassengersCapacity, undefined);
    });

    it('should sort planes by max load capacity', () => {
        airport.sortByMaxLoadCapacity();
        const planesSortedByMaxLoadCapacity = airport.getPlanes();
        const isSorted = planesSortedByMaxLoadCapacity.every((plane, index, array) => {
            if (index < array.length - 1) {
                return plane.getMaxLoadCapacity() <= array[index + 1].getMaxLoadCapacity();
            }
            return true;
        });
    ...
    it('should have at least one bomber in military planes', () => {
        ...
        const hasBomber = bomberMilitaryPlanes.some(plane => plane.getMilitaryType() === MilitaryType.bomber);
        assert.isTrue(hasBomber, "No bomber planes found.");
    });
    ...
    it('should check that experimental planes have classification level higher than unclassified', () => {
        ...
        const hasUnclassifiedPlanes = experimentalPlanes.some(plane => plane.getClassificationLevel() === ClassificationLevel.unclassified);
        assert.isFalse(hasUnclassifiedPlanes, "Unclassified planes found.");
    });
});
